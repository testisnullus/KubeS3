...
func NewItemExponentialFailureRateLimiterWithMaxTries(baseDelay time.Duration, maxDelay time.Duration) workqueue.TypedRateLimiter[reconcile.Request] {
...
func (r *ItemExponentialFailureRateLimiterWithMaxTries) When(item reconcile.Request) time.Duration {
	r.failuresLock.Lock()
...
func (r *ItemExponentialFailureRateLimiterWithMaxTries) NumRequeues(item reconcile.Request) int {
	r.failuresLock.Lock()
	defer r.failuresLock.Unlock()
...
func (r *ItemExponentialFailureRateLimiterWithMaxTries) Forget(item reconcile.Request) {
	r.failuresLock.Lock()
	defer r.failuresLock.Unlock()
